# http://job.haut.edu.cn/module/newsdetail/id-37472
    ...
    def get_url(self,search='人工智能'):
        ...
        for y in range(1, 11):
            try:
                ...
                pass
            url = 'ttp://job.haut.edu.cn/module/newsdetail/id-37472'
            self.driver.get(url)
            ...
            for x in range(1, 30):
                ...
                # 就业渠道
                try:
                    xpath_xueli = '//*[@id="main"]/div/div[2]/ul/li[' + str(
                        x) + ']/div/div[1]/div[1]/div/div[2]/p/text()[2]'
                    WebDriverWait(self.driver, 1, 0.5).until(EC.presence_of_element_located((By.XPATH, xueli)))
                    jingyan_xueli = self.driver.find_element_by_xpath(xpath_xueli).text
                except:
                    channel = ""
...
class WriteDataToCSV:
    def csv_init(self, path):
        ...
        # 3. 构建列表头
        self.csv_writer.writerow(["序号", "生源人数","就业生去向","匹配度", "企业规模", "性质/行业", "岗位名称",
                              "薪酬范围", "学院","专业","就业人数","学院人数"])
        # 4. 关闭文件
        self.f.close()
    def write(self, data):
