...
adv_data = pd.read_excel("../data/twoline.xlsx")
# 清洗不需要的数据
...
sns.pairplot(new_adv_data, x_vars=['fromdata', 'cha'], y_vars='to', height=7, aspect=0.75, kind='reg')
plt.savefig("pairplot.jpg")
plt.show()

# 利用sklearn里面的包来对数据集进行划分，以此来创建训练集和测试集
# train_size表示训练集所占总数据集的比例
X_train, X_test, Y_train, Y_test = train_test_split(new_adv_data[['fromdata','cha']], new_adv_data.to, train_size=.75)

print("原始数据特征:", new_adv_data[['fromdata','cha']].shape,
      ",训练数据特征:", X_train.shape,
      ",测试数据特征:", X_test.shape)

print("原始数据标签:", new_adv_data.to.shape,
